    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                MyApp.Visible = false;
                MyBook = MyApp.Workbooks.Open(@"C:\Users\v-gikal\Desktop\excel.xlsx", Type.Missing, false,
                            Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing,
                            Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
                MySheet = (Excel.Worksheet)MyBook.Sheets[1]; // Explicit cast is not required here
                var lastRow = MySheet.Cells.SpecialCells(Excel.XlCellType.xlCellTypeLastCell).Row;

                BindingList<RateCardPayload> resList = new BindingList<RateCardPayload>();

                for (int index = 1; index <= lastRow; index++)
                {
                    lastRow += 1;
                    MySheet.Cells[lastRow, 1] = payload.Meters[index].MeterCategory.ToString();
                    MySheet.Cells[lastRow, 2] = payload.Meters[index].MeterName.ToString();
                    MySheet.Cells[lastRow, 3] = payload.Meters[index].MeterSubCategory.ToString();
                    MySheet.Cells[lastRow, 4] = payload.Meters[index].MeterRegion.ToString();
                    MySheet.Cells[lastRow, 5] = payload.Meters[index].MeterRates[0].ToString();
                    MySheet.Cells[lastRow, 6] = payload.Meters[index].Unit.ToString();
                    resList.Add(payload);
                    MyBook.Save();
                }
